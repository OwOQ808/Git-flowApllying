  		...
  		if(xml.responseText==0){
			...
			xmll.onreadystatechange = function(){
				...
				if (xmll.readyState !== 4 || xmll.status !== 200) {
  				...
  				elem3.appendChild(document.createTextNode('Email: '+ info[0][0].substring(0, 3) +
				 info[0][0].substring(3).replace(/[\s\S]/g, "*") ));
  				elem2.appendChild(elem3);

  				elem3=document.createElement('div');
  				elem3.appendChild(document.createTextNode('Логин: '+ info[0][1].substring(0, 3) +
				 info[0][1].substring(3).replace(/[\s\S]/g, "*") ));
  				elem2.appendChild(elem3);

  				elem3=document.createElement('div');
  				elem3.appendChild(document.createTextNode('Телефон: '+ info[0][4].substring(0, 3) +
				 info[0][4].substring(3).replace(/[\s\S]/g, "*") ));
  				elem2.appendChild(elem3);
...
function loadMain(){
	...
	xml.onreadystatechange = function(){
		if (xml.readyState !== 4 || xml.status !== 200) {
  		...
  		new MainSlider('/resourses/images/book.jpg','/resourses/images/book1.jpg',
			'/resourses/images/book2.jpg').createSlider(document.getElementById('booksimages').querySelectorAll('div')[0]);
